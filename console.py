...
class HBNBCommand(cmd.Cmd):
    ...
    def do_create(self, line):
        ...
        if not validate_classname(arguments, check_id=True):
            return

        new_object = current_classes[arguments[0]]()
        new_object.save()
        print(new_object.id)
    ...
    def do_show(self, line):
        ...
        arguments = line.split()
        if not validate_classname_with_id(arguments, check_id=True):
            return

        instance_object = storage.all()
        key = "{}.{}".format(arguments[0], arguments[1])
        req_instance = instance_object.get(key, None)
    ...
    def do_destroy(self, line):
        """Delets an instance based on the class ame and id."""
        arguments = line.split()
        if not validate_classname_with_id(arguments, check_id=True):
            return
        instance_object = storage.all()
    ...
    def do_all(self, arg):
        ...
        if len(args) < 1:
            print(["{}".format(str(v)) for k, v in all_objs.items()])
            return
...
def validate_classname(arguments, check_id=False):
    if len(arguments) < 1:
...
def validate_classname_with_id(arguments, check_id=False):
    if len(arguments) < 1:
        print("** class name missing **")
        return False
    if arguments[0] not in current_classes.keys():
